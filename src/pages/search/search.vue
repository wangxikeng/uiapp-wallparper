<template>
  <view class="searchLayout">
    ...
    <view>
      <view class="list" v-show="!noSearch">
        <navigator
...
import { apiSearchData } from "@/api/api";
...
const searchData = async () => {
  try {
    ...
    if (!res.data.length && !classList.value.length) {
      noSearch.value = true;
    }
...
const initData = (value) => {
  queryParams.value = { pageNum: 1, pageSize: 12, keyword: value || "" };
  classList.value = [];
  noData.value = false;
